...
@section('content')
    ...
    {{-- <script>
        $(document).ready(function() {
            ...
            $('#start_date, #end_date, #type_id').change(function() {
                calculateDuration();
                checkDurationAgainstMaxLeave();
            });
            ...
            function checkDurationAgainstMaxLeave() {
                var selectedTypeId = $('#type_id').val();
                console.log('Tipe yang dipilih : ', selectedTypeId);
                var duration = parseInt($('#duration').val());
                console.log('Jumlah Hari : ', duration);

                if (!isNaN(duration) && duration > 0) {
                    // Lakukan request AJAX untuk mendapatkan informasi jenis cuti dari backend
                    $.ajax({
                        type: 'GET',
                        url: '/get-leave-type/' + selectedTypeId,
                        success: function(response) {
                            var maxLeave = response.max_leave;
                            if (duration >= maxLeave) {
                                if (duration === maxLeave) {
                                    Swal.fire({
                                        icon: 'warning',
                                        title: 'Perhatian!',
                                        text: ' Jumlah Cuti tidak bisa dipakai dalam  1 waktu bersamaan',
                                    });
                                } else {
                                    Swal.fire({
                                        icon: 'error',
                                        title: 'Oops...',
                                        text: 'Jumlah hari melebihi jumlah cuti dari jenis yang Anda pilih!',
                                    });
                                }
                            }
                        },
                        ...
                        error: function(xhr, status, error) {
                            Swal.fire({
                                icon: 'error',
                                title: 'Oops...',
                                text: 'Terjadi kesalahan saat memeriksa jumlah hari!',
                            });
                        }
                    });
                }
